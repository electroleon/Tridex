...
class ZCalibrationHelper:
    def __init__(self, config):
        ...
        self.tool_gcode = gcode_macro.load_template(config, 'tool_gcode', '')
        self.return_gcode = gcode_macro.load_template(config, 'return_gcode', '')		
...
class CalibrationState:
    ...
    def _add_probe_offset(self, site):
        ...
        return probe_site
    def _set_new_gcode_offset(self, offset, offsett1):
        ...
        command_string = ("SET_GCODE_VARIABLE MACRO=DC_VARS VARIABLE=offset_zt0 VALUE=%s"
                         % (offset))
        self.gcode.run_script_from_command(command_string)
        command_string = ("SET_GCODE_VARIABLE MACRO=DC_VARS VARIABLE=offset_zt1 VALUE=%s"
                         % (offsett1))
        self.gcode.run_script_from_command(command_string)
        #self.gcode.run_script_from_command("SET_GCODE_VARIABLE MACRO=DC_VARS VARIABLE=offset_zT0 VALUE="0)
    def calibrate_z(self):
        ...
        self.helper.end_gcode.run_gcode_from_command()
        self.helper.tool_gcode.run_gcode_from_command()		
		
        nozzle_zerot1 = self._probe_on_site(self.z_endstop,
                                          self.helper.nozzle_site)
        offsett1 = probe_zero - (switch_zero - nozzle_zerot1
                               + self.helper.switch_offset)
		
        self.gcmd.respond_info("Z-CALIBRATION: ENDSTOP=%.3f NOZZLE=%.3f"
                               " SWITCH=%.3f PROBE=%.3f --> OFFSET=%.6f"
                               % (self.helper.z_homing, nozzle_zerot1,
                                  switch_zero, probe_zero, offsett1))		
        self.helper.return_gcode.run_gcode_from_command()
        # set new offset
        self._set_new_gcode_offset(offset,offsett1)
        # set states
